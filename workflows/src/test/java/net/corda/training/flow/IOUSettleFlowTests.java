...
public class IOUSettleFlowTests{
    ...
    private Cash.State issueCash(String currency, long amount) throws InterruptedException, ExecutionException {
        SelfIssueCashFlow flow = new SelfIssueCashFlow(currency,amount);
        CordaFuture future = a.startFlow(flow);
        mockNetwork.runNetwork();
...
//
//        SignedTransaction stx = issueIOU("GBP", 10, b.getInfo().getLegalIdentities().get(0), a.getInfo().getLegalIdentities().get(0));
//        issueCash("GBP", 5);
//
//        IOUState inputIOU = stx.getTx().outputsOfType(IOUState.class).get(0);
//        IOUSettleFlow.InitiatorFlow flow = new IOUSettleFlow.InitiatorFlow(inputIOU.getLinearId(),"GBP", 5);
